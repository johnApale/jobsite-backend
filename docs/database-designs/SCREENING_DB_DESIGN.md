...
| --------------- | ------------ | ----------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| id              | uuid         | PK                                                    |                                                                                                                                        |
| application_id  | uuid         | NOT NULL, FK → recruitment.applications.id            | The application this answer belongs to                                                                                                 |
| question_id     | uuid         | NOT NULL, FK → recruitment.job_screening_questions.id | The question being answered. Cross-schema FK to Recruitment                                                                            |
...
recruitment.job_screening_questions ||--o{ screening_question_responses   : "has many (cross-schema)"
...
`screening_question_responses` has a cross-schema FK to `recruitment.job_screening_questions`. This is intentional — referential integrity for question answers matters more than strict schema isolation.
...
**`screening_question_responses` in the Screening schema with cross-schema FK.** Answers belong to the screening/evaluation domain, not to Recruitment. The cross-schema FK to `recruitment.job_screening_questions` ensures referential integrity. Storing answers in Screening keeps the evaluation data together and avoids splitting the scoring data across modules.

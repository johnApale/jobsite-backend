...
Lock the shortlist. Updates application statuses to "Shortlisted" and publishes `CandidateShortlistedEvent` for each active candidate.
...
| Event                       | Source    | Action                                        |
| --------------------------- | --------- | --------------------------------------------- |
| `CvScreeningCompletedEvent` | Screening | Creates `CandidateMatch` with screening score |

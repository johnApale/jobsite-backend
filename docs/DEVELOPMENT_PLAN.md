...
- **Phase 3 complete** — Profiles module implemented: applicant profile CRUD (JSON merge patch), resume upload with file storage abstraction, async resume parsing via MassTransit/RabbitMQ (basic parsing synchronous + AI parsing via broker: `ResumeParseRequested` → `ResumeParsed`), `UserRegisteredEvent` handler auto-creates profiles for Applicants
...
- **Phase 5 complete** — Screening module implemented: dual scoring engine (deterministic always + AI opt-in via broker), three-tier routing (auto-advance/manual-review/auto-reject), screening question response scoring (MultipleChoice/YesNo deterministic + FreeText via broker: `AnswerScoringRequested` → `AnswersScored`), assessment flow for AfterScreening questions, candidate transparency feedback via broker (`FeedbackGenerationRequested` → `FeedbackGenerated`), manual review workflow, `ApplicationSubmittedEvent` handler triggers screening pipeline, `AssessmentCompletedEvent` published to Admin audit log, 6 API endpoints with cursor-based pagination
...
- Message broker integration (monolith ↔ AI Service) — RabbitMQ messaging implemented for 4 async operations (resume parsing, screening evaluation, answer scoring, candidate feedback); 2 sync operations remain HTTP (criteria suggestion, question suggestion)
- AI Service endpoints: criteria generation, assessment question generation remain HTTP; resume parsing, AI screening, answer scoring, candidate feedback use RabbitMQ consumers
...
| 3.7 | **AI Service broker integration** | Infrastructure          | Publish `ResumeParseRequested` to message broker after basic parsing; AI Service consumes, extracts structured data (skills with levels/years, experience, education, certifications), publishes `ResumeParsed` back |
| 3.8 | **Resume parsed consumer**        | Infrastructure          | `ResumeParsedConsumer` receives `ResumeParsed` from broker; stores AI-extracted data in `ai_parsed_content` on resume record                                                                                         |
| 3.9 | **Tests**                         | Tests                   | Profile creation linked to auth user, resume versioning, parsing pipeline, AI parse fallback                                                                                                                         |

**Exit Criteria:** Applicants have profiles linked 1:1 with auth users. Resumes are uploaded, versioned, and parsed asynchronously. Basic parsing runs synchronously; AI parsing is async via message broker (`ResumeParseRequested` → `ResumeParsed`). AI-parsed structured data is stored when the broker response arrives.
...
| 5.5  | **AI scoring service**                | Application             | When enabled (system gate + tenant `ai_scoring_enabled`): publish `ScreeningEvaluationRequested` to message broker; AI Service consumes, evaluates, publishes `ScreeningEvaluated` back; `ScreeningEvaluatedConsumer` applies `ai_criteria_score_breakdown` and `ai_overall_score`. When disabled: fields stay null                     |
| 5.6  | **Question scoring service**          | Application             | Score AtApplication question answers: MultipleChoice/YesNo deterministically (always); FreeText via message broker: publish `AnswerScoringRequested`, AI Service publishes `AnswersScored` back; `AnswersScoredConsumer` applies scores to response records (independent of AI scoring flag)                                            |
...
| 5.10 | **Candidate transparency**            | Application             | When enabled (tenant `candidate_transparency_enabled`): publish `FeedbackGenerationRequested` to message broker; AI Service publishes `FeedbackGenerated` back; `FeedbackGeneratedConsumer` stores `candidate_feedback` on screening result                                                                                             |
...
| 6.4  | **Resume parsing consumer**          | Core / Messaging              | Consume `ResumeParseRequested` from RabbitMQ — parse resume text, extract structured data (skills with levels/years, experience, education, certifications), cache by file hash, publish `ResumeParsed` back |
...
| 6.7  | **AI screening evaluation consumer** | Core / Messaging              | Consume `ScreeningEvaluationRequested` from RabbitMQ — evaluate parsed resume against criteria, produce per-criterion AI scores with reasoning, publish `ScreeningEvaluated` back                            |
| 6.8  | **Answer scoring consumer**          | Core / Messaging              | Consume `AnswerScoringRequested` from RabbitMQ — score question answers with rubric, publish `AnswersScored` back                                                                                            |
| 6.9  | **Candidate feedback consumer**      | Core / Messaging              | Consume `FeedbackGenerationRequested` from RabbitMQ — generate candidate-facing feedback summary, publish `FeedbackGenerated` back                                                                           |
...
> **Note:** AI Interview endpoints (session management, question delivery, response submission, media transcription), broker consumer/publisher (`CandidateReadyForInterviewEvent` / `InterviewCompletedEvent`), and interview database tables are **deferred** to a future release. See TODO.md for the deferred items list. The RabbitMQ infrastructure for the 4 async operations (resume parsing, screening evaluation, answer scoring, candidate feedback) has been implemented in the monolith — Phase 6 needs to implement the corresponding AI Service consumers and publishers.

**Exit Criteria:** AI Service provides criteria generation and assessment question suggestions via HTTP endpoints. Resume parsing, AI screening evaluation, answer scoring, and candidate feedback generation operate via RabbitMQ message broker (consume request events, publish response events). All AI provider calls are logged for cost tracking. Resume parse results are cached by file hash.
